...
// Model for Posicion entity
    models.Posicion = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Sigueme/posiciones/",
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/Sigueme/posiciones/';
            }
    ...
    // Collection class for Posicion entities
    models.PosicionCollection = Backbone.Collection.extend({
        model: models.Posicion,
        url: "http://localhost:8080/Sigueme/posiciones/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PosicionCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-posicion-list-itemis template identifier for item
                        templateName: '#tpl-posicion-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.posicion = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.posicion,
                    // tpl-posicion-details is template identifier for chosen model element
                    templateName: '#tpl-posicion-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Posicion();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-posicion-details is a template identifier for chosen model element
                templateName: '#tpl-posicion-details',
